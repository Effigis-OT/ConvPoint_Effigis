...
print("")
type_loss = input("Quel type de loss doit être utilisé (C pour Cross-entropy, F pour Focal, CF pour les deux? \n").upper()
if type_loss == "C":
    code_loss = "ce"
elif type_loss == "F":
    code_loss = "fl"
...
os.system("python {0} --savedir {1} --rootdir {2} --testdir {3} --resdir {4} --model_state {5} --dsname {6} --lr {7} "
          ...
          "--finetune {14} --decapit {15} --mode {16} --loss {17}".format(script_train, doss_modele, doss_datasets,
                                                                          doss_test, doss_res, modele_pth, nom_entrain,
                                                                          lr0, nbpoints0, blocksize0, iter0, nbepochs0,
                                                                          batchsize0, nbworkers0, option_finetune,
                                                                          option_decapit, code_classes, code_loss))
